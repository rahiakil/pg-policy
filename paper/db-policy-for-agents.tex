...
\textbf{Contributions:} (1)~a survey of in-DB and adjacent policy systems against agentic requirements; (2)~APL and pg\_agent\_policy~\cite{pgagentpolicy} as an industrial artifact; (3)~latency and expected-loss analysis that separates RLS row-path cost from once-per-tool PDP cost; (4)~SQL examples and domain packs.
...
Agents make the database a tool. Index RLS for rows. Evaluate APL once per tool. Do not accept ``the database is too slow for policy'' without asking \emph{which plane}. Artifact: \url{https://github.com/rahiakil/pg-agent-policy}.
