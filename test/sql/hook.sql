...
-- Proves the hook is non-bypassable: a connection that never calls
-- evaluate() still hits the referee. Enforcement is active only when
-- a principal is bound via the trusted GUC (agent session); admin
-- sessions (no GUC) are not enforced.
...
-- 1. Admin mode (no principal GUC): DDL is allowed.
CREATE TABLE admin_ok (id int);
DROP TABLE admin_ok;

-- 2. Agent mode: bind principal, CREATE is blocked by the hook.
...
-- 4. After unbinding, admin DDL works again.
