...
SELECT agent_policy.upsert_policy('pack_health_need_actor', $apl$
forbid
  principal agent "*"
  action tool "*"
...
SELECT agent_policy.upsert_policy('pack_health_no_export', $apl$
forbid
  principal agent "*"
...
SELECT agent_policy.upsert_policy('pack_health_no_copy', $apl$
forbid
  principal agent "*"
...
SELECT agent_policy.upsert_policy('pack_health_read', $apl$
permit
  principal agent "*"
  action tool "execute_sql"
...
SELECT agent_policy.upsert_policy('pack_health_min_nec', $apl$
guide
  principal agent "*"
  action tool "execute_sql"
