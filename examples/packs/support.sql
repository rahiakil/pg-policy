...
SELECT agent_policy.upsert_policy('pack_support_pii_export', $apl$
forbid
  principal agent "*"
...
SELECT agent_policy.upsert_policy('pack_support_refund_quota', $apl$
forbid
  principal agent "*"
  action tool "refund"
  when temporal {
...
SELECT agent_policy.upsert_policy('pack_support_refund_unapproved', $apl$
forbid
  principal agent "*"
  action tool "refund"
  when { context.approved == "false" }
...
SELECT agent_policy.upsert_policy('pack_support_read_sql', $apl$
permit
  principal agent "*"
...
SELECT agent_policy.upsert_policy('pack_support_steer', $apl$
guide
  principal agent "*"
  action tool "execute_sql"
